...
export class AccountTypeCoreService {
    ...
    static async findOne(
        filters: QueryAccountTypes,
    ): Promise<SelectAccountType | null> {
        const conditions = AccountTypeCoreService.buildWhereConditions(filters)
        const accountTypes = await db
            .select()
            .from(accountTypesTable)
    ...
    static async update(
        id: string,
        input: Partial<InsertAccountType>,
    ): Promise<SelectAccountType> {
        const [accountType] = await db
    ...
    static async deleteMany(ids: string[]): Promise<void> {
        await db
            .delete(accountTypesTable)
            .where(inArray(accountTypesTable.id, ids))
    }

    static buildWhereConditions(
        params: QueryAccountTypes,
